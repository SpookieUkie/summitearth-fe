...
import { DfrRRRService } from 'src/app/dfrRRR/dfr-rrr.service';


@Component({
  selector: 'app-rrrdfrmanagement',
  templateUrl: './rrrdfrmanagement.component.html',
  styleUrls: ['./rrrdfrmanagement.component.scss'],
})
export class RrrdfrmanagementComponent implements OnInit {
  viewWidth = 1000;
  ...
  async getDfrs() {
    const loader = await this.loadingController.create({
      ...
      this.dfrRRRservice.getDFRListByDate(this.form.value.startDate, this.form.value.endDate).subscribe( (val: any) => {
        this.dfrs = val;
  ...
  copyDFR(item: DfrModel) {
    ...
    this.dfrRRRservice.copyDFR(dfr._id).subscribe(val => {
       this.sharedService.showNotification('Daily Field Report - RRR Copied');
